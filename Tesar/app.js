...
function navToggle(e) {
  if (!e.target.classList.contains("active")) {
    ...
    gsap.to("#logo", 1, { color: "rgb(167, 110, 231)" });
...
function detailAnimation() {
  ...
  slides.forEach((slide, index, slides) => {
    ...
    slideTl.fromTo(nextImg, { x: "100%" }, { x: "0%" }, "-=0.5");
    //Scene
...
function hikeAnimation() {
  ...
  slides.forEach((slide, index, slides) => {
    ...
    slideTl.fromTo(slide, { opacity: 1 }, { opacity: 0 });
    slideTl.fromTo(nextSlide, { opacity: 0 }, { opacity: 1 }, "-=1");
    slideTl.fromTo(nextImg, { x: "150%" }, { x: "0%" }, "-=0.5");
    //Scene
...
function climbingAnimation() {
  controller = new ScrollMagic.Controller();
  const slides = document.querySelectorAll(".climbing-slide");
  slides.forEach((slide, index, slides) => {
    const slideTl = gsap.timeline({ defaults: { duration: 1 } });
    let nextSlide = slides.length - 1 === index ? "end" : slides[index + 1];
    const nextImg = nextSlide.querySelector("img");
    ...
    slideTl.fromTo(nextImg, { x: "100%" }, { x: "0%" }, "-=0.5");
    //Scene
    climbingScene = new ScrollMagic.Scene({
      triggerElement: slide,
      duration: "100%",
      triggerHook: 0,
    })
